...
def ensure_ready_for_upsampling(x, c):
    right_size = len(c) * audio.get_hop_size()
    if len(x) < right_size:
        x = _pad(x, right_size)
    elif len(x) > right_size:
        x = x[:right_size]
    return x
...
def collate_fn(batch):
    ...
    local_conditioning = len(batch[0]) >= 3 and hparams.cin_channels > 0
    global_conditioning = len(batch[0]) >= 4 and hparams.gin_channels > 0
